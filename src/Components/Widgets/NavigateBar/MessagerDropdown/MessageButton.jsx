...
import { inscreaseUnread, setIsOpenChat, updateOneChat } from '../../../../Redux/MessagerSlice';

export default function MessageButton({ isOpen }) {

    const dispatch = useDispatch();
    const unread = useSelector(state => state.messenger.unread);

    useEffect(() => {
        socket.on(`receiver`, (data) => {
            dispatch(updateOneChat(data));
            dispatch(inscreaseUnread());
        });
    ...
    return (
        <div onClick={() => dispatch(setIsOpenChat(!isOpen))} className={` w-10 h-10 flex justify-center items-center relative bg-gray-300 rounded-full hover:bg-gray-400 active:ring-gray-300 active:ring cursor-pointer`}>
            ...
            {unread > 0 && (
                <div className=' absolute w-4 h-4 text-white top-0 right-0 p-2 flex justify-center items-center bg-red-600 text-sm font-semibold  rounded-full'>{unread}</div>
            )}
        </div>
    )
}
