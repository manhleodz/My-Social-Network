...
export default function Search() {
    ...
    const search = () => {
        ...
        if (searchWord === "") {
            ...
            setInput(searchWord);
            SearchAPI.search(searchWord, page).then((res) => {
                if (res.status === 200) {
                    page = res.data.page;
    ...
    return (
        <>
            <div>
                ...
                <div className={`${input.length !== 0 ? 'absolute left-0 top-0 z-50 w-96 shadow-2xl rounded-lg bg-white outline-none space-y-3  p-3' : ''}`}>
